...
export function isCareful(cwd: string): boolean {
	return readJSON<CarefulState>(statePath(cwd, "careful.json"), {
		active: false,
	}).active;
}
...
export default function carefulMode(pi: ExtensionAPI) {
	...
	pi.registerCommand("careful", {
		...
		handler: async (args, ctx) => {
			...
			const file = statePath(ctx.cwd, "careful.json");
			if (arg === "on") {
				writeJSON(file, { active: true, since: new Date().toISOString() });
				ctx.ui.notify("careful mode ON — destructive commands will be blocked", "warn");
			} else if (arg === "off") {
				writeJSON(file, { active: false });
