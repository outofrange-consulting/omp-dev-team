...
| Plugin | Rôle |
|---|---|
| **[`dev-team`](plugins/dev-team/)** | **Équipe de dev agentique** — un orchestrateur + 30 agents spécialistes/critiques, le workflow `/specs` → `/dt-plan` → `/build` → `/pr`, un **plan gate strict** (test-after, tests requis) et points de contrôle humains, 64 skills, des extensions « garde-fou » bloquantes, et un gate `dotnet build` + tests ciblé après édition C#. Portage de [bdfinst/agentic-dev-team](https://github.com/bdfinst/agentic-dev-team) (Bryan Finster), suivi jusqu'à la v10.20.0. Ouvert sur les modèles : les agents passent par les rôles OMP (`@smol`/`@task`/`@plan`/`@slow`), jamais par un id de fournisseur en dur. |
...
JSON valide ; les 12 extensions de dev-team et celles de tous les autres plugins
