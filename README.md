...
| Plugin | What it does |
|---|---|
| **[`dev-team`](plugins/dev-team/)** | **Agentic dev team** — orchestrator + 30 specialist/critic agents, the `/specs` → `/dt-plan` → `/build` → `/pr` workflow, a **forced plan gate** (test-after, tests required) and human gates, 64 skills, blocking guard extensions, and a scoped `dotnet build` + test gate after C# edits. Port of [bdfinst/agentic-dev-team](https://github.com/bdfinst/agentic-dev-team) (Bryan Finster), tracked to v10.20.0. Model-open: agents route through OMP roles (`@smol`/`@task`/`@plan`/`@slow`), never a hardcoded vendor id. |
