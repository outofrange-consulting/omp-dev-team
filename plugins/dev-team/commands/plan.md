...
2. Decompose the feature into **vertical slices**; author each slice's **Gherkin
   scenarios** (the behavioral contract) and its **test list** before
   implementation (tests are required; test-first ordering is not). Each slice
   declares `Depends-on` + `Files`; derive the `## Parallelization` waves so
   independent slices can build concurrently.
...
then run **`/plan-approve`** to unlock implementation (the `plan-gate` extension
blocks source edits until then). For a genuinely trivial task, `/scope --trivial`
instead of planning.
