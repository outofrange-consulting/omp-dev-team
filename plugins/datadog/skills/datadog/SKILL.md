...
name: datadog
description: >-
  Query and operate Datadog from the terminal: logs, metrics, traces/APM,
  monitors and alerts, incidents, dashboards, SLOs, synthetics, RUM, security and
  audit trail, CI/CD test visibility and pipeline triage, agent/SSI install, and
  LLM/agent observability. Use whenever the user mentions Datadog (or DD), asks to
  investigate an alert/incident/error/latency/log/trace/flaky test, check service
  health, or manage Datadog monitors/dashboards/SLOs. This is the single entry
  point — it drives the Datadog `pup` CLI, which carries the per-domain skills.
---
...
Power users can install pup's domain skills as first-class OMP skills with
`bash plugins/datadog/install.sh --with-skills` (`pup skills install pi`). It's
off by default to keep the skill surface small — this umbrella plus the `pup`
CLI already covers the full product.
