...
description: >-
  Query the codebase's persistent knowledge graph instead of grep+read. Use for "where is X", "who calls X", "what does X call", "impact of changing X", and architecture/flow questions — far fewer tokens and tool calls. For precise C# semantic operations (rename, exact references, diagnostics, hover), use the csharp-ls LSP instead.
---
