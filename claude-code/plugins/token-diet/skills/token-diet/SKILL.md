...
name: token-diet
description: >-
  How this workspace minimizes LLM token spend. Use when the user asks how to reduce tokens/cost, what ctx-wire / codebase-memory-mcp / caveman / yagni are, how to set them up, or whether OMP already does X natively.
---
...
- "Where is X / who calls X / what breaks if I change X / how does this module
  fit together?" → use `the /codebase-memory skill` (the codebase-memory-mcp tools:
...
- Want the agent to **write minimal code** (YAGNI, no over-engineering) or to
  review a diff for bloat → `/yagni` (`the /yagni skill`).
...
`the /codebase-memory skill`) are enabled in `~/.omp/agent/config.yml`. Pass
