...
name: triage
description: >-
  Investigate a bug, find its root cause, and write a portable triage record to .triage/<slug>.md with a fix plan (regression test + fix + refactor). Use when the user reports a bug and wants it triaged, says "triage this", "investigate and write it up", or wants a hands-off bug investigation that produces an actionable record.
...
Apply the systematic debugging protocol from `the /systematic-debugging skill`:
