...
| `--init-risks` | Scaffold `ACCEPTED-RISKS.md` from `templates/ACCEPTED-RISKS.md.tmpl` if absent. Exits non-zero without overwriting if present. Schema: `dev-team-knowledge/accepted-risks-schema.md`. |
...
…**except functional Claude-config markdown, which is never documentation** (it drives agent/skill/command behavior and must be reviewed): any path containing a `.claude/` segment, or under `agents/`, `skills/`, `prompts/`, `dev-team-knowledge/`, or `templates/agents/`. Treat `CLAUDE.md` and `AGENTS.md` as functional config too, not documentation.
...
Pass availability info to each agent so they can use enhanced tools or fall back to Glob/Grep/Read. Include in the final report per `dev-team-knowledge/review-template.md`.
...
Follow the detection, execution, and deduplication procedure in [`the /static-analysis-integration skill`](../the /static-analysis-integration skill). Output is structured findings injected into agent context in step 4. **This step does not gate execution** — it collects context only.
...
- **Static analysis context**: if step 2b produced findings, inject into every agent's prompt using the format in `the /static-analysis-integration skill`: "These issues were detected by static analysis. Do not re-report them. Focus on semantic concerns."
...
If `ACCEPTED-RISKS.md` exists at the repo root, parse its `rules:` YAML frontmatter per `dev-team-knowledge/accepted-risks-schema.md`. For each finding, check rules in declaration order; the first match suppresses and emits one audit entry:
...
Read `dev-team-knowledge/review-rubric.md` for the formula. Compute the overall health score; security failures auto-escalate to 🔴.
...
Read `dev-team-knowledge/review-template.md` for the structure.
