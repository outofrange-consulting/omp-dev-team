...
- [Quality Gate Pipeline](the /quality-gate-pipeline skill) - invoke before delivery (Phase 1: self-validation), before completion claims (Phase 2: verification evidence), and during rework (Phase 3: review-correction loop)
- [Testing Discipline](the /testing-discipline skill) - invoke when writing or modifying code: tests are required (test-first optional), verified green by `/impl-verify` before a unit is done
- [Systematic Debugging](the /systematic-debugging skill) - invoke when any test fails or unexpected behavior occurs; no guess-and-fix
- [Hexagonal Architecture](the /hexagonal-architecture skill) - invoke when structuring new services or modules with port/adapter separation
- [Domain-Driven Design](the /domain-driven-design skill) - invoke when modeling business domains, defining aggregates, or mapping bounded contexts
- [API Design](the /api-design skill) - invoke when implementing APIs to verify contract compliance
- [Legacy Code](the /legacy-code skill) - invoke when modifying or extending code that lacks test coverage or has poor structure
- [Mutation Testing](the /mutation-testing skill) - invoke when assessing whether tests for new or modified code are catching meaningful faults
- [Code Review](the /code-review skill) - invoked by orchestrator after each discrete unit of work and before committing; do not invoke independently
