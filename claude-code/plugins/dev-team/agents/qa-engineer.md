...
- Test case generation (unit, integration, e2e) derived from the plan's slice scenarios — judged against `dev-team-knowledge/test-automation-principles.md` (goals + principles rubric); DB tests follow `dev-team-knowledge/database-test-patterns.md` (Fake vs real DB, isolation, teardown)
...
- [Quality Gate Pipeline](the /quality-gate-pipeline skill) - invoke before delivery (Phase 1: self-validation), before signing off (Phase 2: verification evidence), and during peer validation or rework (Phase 3: review-correction loop)
- [Testing Discipline](the /testing-discipline skill) - invoke when generating tests: cover behavior + edge/error cases, real code over mocks, verified by `/impl-verify`
- [Systematic Debugging](the /systematic-debugging skill) - invoke when investigating test failures or defects; enforce 4-phase protocol
- [Specs](the /specs skill) - invoke after the consistency gate passes; the spec sets intent, architecture, and acceptance criteria. The per-slice Gherkin you treat as acceptance-test contracts is authored in `/plan`.
- [Legacy Code](the /legacy-code skill) - invoke when writing characterization tests to lock down existing legacy behavior before changes
- [Mutation Testing](the /mutation-testing skill) - invoke when evaluating test suite effectiveness or validating that tests catch behavioral changes
- [Test Review](the /test-review skill) - delegate test file analysis to this review agent rather than duplicating its checks; invoke via `/review-agent test-review` when reviewing test quality inline
