...
description: Generate production-ready Dockerfiles from project source. Detects language/framework automatically and produces multi-stage builds with distroless, slim, or minimal base images. Use to containerize an app, write a Dockerfile, or rewrite an existing one for production.
model: claude-sonnet-4.6
metadata:
  tier: balanced
---

# docker-image-create — production-ready Dockerfiles
...
**Default base-image strategy:** distroless when possible (only the app and its runtime — no shell, package manager, or OS utilities; minimal CVE exposure). Fall back to slim (`node:22-slim`, `python:3.12-slim`) when the runtime needs OS-level libraries or a shell for debugging.
...
1. **Language + version** (from version files/configs): `package.json` (Node — `engines.node`), `go.mod` (Go), `requirements.txt`/`pyproject.toml`/`Pipfile` (Python), `pom.xml`/`build.gradle{,kts}` (Java/Kotlin), `*.csproj`/`*.fsproj` (C#/F# — `TargetFramework`), `mix.exs` (Elixir), `Gemfile` (Ruby).
2. **Framework** (drives build command + output structure): Node (Next.js, Remix, Express, Fastify, NestJS, Astro, SvelteKit); Python (FastAPI, Flask, Django); Go (net/http, Gin, Echo, Fiber); Java (Spring Boot, Quarkus, Micronaut); .NET (ASP.NET Core, Blazor, gRPC, minimal APIs).
3. **Package manager** (drives install command): Node — npm/yarn/pnpm/bun (by lockfile); Python — pip/poetry/pipenv/uv (by lockfile).
...
Present a detection summary and ask: "Want me to adjust anything before generating?"
...
2. **Build tool?** Default: BuildKit (`DOCKER_BUILDKIT=1`) — enables cache mounts, secret mounts, parallel stages.
...
**Layer order** (least-changed → most-changed) for cache efficiency: base image → system deps → dependency manifest + install → source → build → final stage with only the runtime artifact.
...
If absent or sparse, write one tailored to the detected language:
...
- Use the `node` user (built into slim images).
...
### C# / .NET
- Build `mcr.microsoft.com/dotnet/sdk:<version>`; runtime `mcr.microsoft.com/dotnet/aspnet:<version>-alpine`.
...
- `USER app` (built-in non-root in .NET 8+), `ASPNETCORE_URLS=http://+:8080`.
...
- `jlink` for a custom minimal JRE.
