...
description: >-
  Create Mermaid diagrams using the project's blue-gray theme. Use whenever the
  user asks to draw a diagram, create a flowchart, visualize a process, document
  architecture, or add a sequence/state/ER diagram to a markdown file.
model: claude-haiku-4.5
metadata:
  tier: small
---

# mermaid-diagramming — themed Mermaid diagrams
...
| `primaryBorderColor` / `nodeBorder` | `#3b82f6` / `#2563eb` | Node border |
...
1. **Always include the theme directive** — no exceptions; every ```` ```mermaid ```` block starts with the `%%{init}%%` line.
2. **One diagram per concept.** For multiple views, use separate fenced blocks with a heading for each.
3. **Keep labels short** — node labels ≤ 6 words. Use subgraphs for grouping, not long labels.
4. **Decision nodes use `{...}` rhombus.** Action nodes use `[...]` rectangles. Terminal/IO nodes use `[/..../]` parallelograms.
5. **Left-to-right for pipelines, top-to-bottom for hierarchies.** Default `LR` for data flow; `TD` for org charts, call trees, and phase sequences.
...
1. Read the file first to understand structure and find the right insertion point.
2. Use a second-level heading (`##`) above the diagram block to name the concept.
3. Add a one-sentence caption below the closing fence describing what the diagram shows — this doubles as alt-text.
